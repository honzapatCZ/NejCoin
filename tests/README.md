...
Core tests take longer than any other Nejcoin tests, due to the high amount of computational work involved in validating core components.
...
To run only Nejcoin's core tests (after building):
...
To run only Nejcoin's crypto tests (after building):
...
```bash
nejcoind --regtest --offline --fixed-difficulty 1
```
...
Open the wallet file with `nejcoin-wallet-rpc` with RPC port 18083. Finally, start tests by invoking ./blockchain.py or ./speed.py
...
To run only Nejcoin's hash tests (after building):
...
To run only Nejcoin's performance tests (after building):
...
To run only Nejcoin's unit tests (after building):
